...
- **All packages now declare `Apache-2.0`.** The project has always shipped under Apache-2.0 (see `LICENSE`), but that was declared in neither `packages/*/Cargo.toml` nor any `package.json`, so published tarballs carried no license field and npm would treat them as unlicensed. Both sides now declare it. Added `deny.toml` — previously absent entirely, which made `cargo deny check` fall back to a default policy that rejected even plain MIT/Apache-2.0 — with an exhaustive allow-list (`Apache-2.0`, `MIT`, `Unicode-3.0`, `ISC`) so any GPL/LGPL/AGPL crate entering the graph fails the build, plus an `allow-git` list naming the two rev-pinned first-party Senzing repos as the only permitted git sources. A `deny` job in `security.yml` keeps it enforced.
- Regenerated the committed NAPI-RS loaders (`packages/{sdk,configtool}/index.js` and `index.d.ts`) against the pinned `@napi-rs/cli` 3.10.0. They had been generated by 3.8.5 and were never refreshed when the CLI was bumped, so the tree disagreed with its own lockfile; no CI job diffs these files, so this went unnoticed. Shipped artifacts were unaffected — every release path runs `napi build` first.
